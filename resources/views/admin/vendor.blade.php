...
<body class="flex bg-gray-100 min-h-screen">
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        ...
        {{-- Form Upload Excel --}}
        <form action="{{ route('admin.vendors.import') }}" method="POST" enctype="multipart/form-data" class="mb-6">
            @csrf
        ...
        {{-- Search & Filter --}}
        <form action="{{ route('admin.vendors.search') }}" method="GET" class="flex flex-wrap gap-2 mb-6">
            <input type="text" name="query" value="{{ request('query') }}" placeholder="Cari Vendor atau PIC" class="border border-gray-300 rounded px-3 py-2 flex-1 min-w-[200px]">
            <select name="sort" class="border border-gray-300 rounded px-3 py-2">
                <option value="">Urutkan</option>
                <option value="name" {{ request('sort') == 'name' ? 'selected' : '' }}>Nama Perusahaan (A-Z)</option>
                <option value="pic_name" {{ request('sort') == 'pic_name' ? 'selected' : '' }}>Nama PIC (A-Z)</option>
            </select>
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">
                🔍 Cari
            </button>
        ...
        {{-- Form Hapus Banyak --}}
        <form action="{{ route('admin.vendors.bulkDelete') }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus vendor yang dipilih?')">
            @csrf
            @method('DELETE')
                        ...
                        @forelse($vendors as $vendor)
                            <tr class="hover:bg-gray-50">
                                <td class="border px-4 py-2 text-center">
                                    <input type="checkbox" name="selected_vendors[]" value="{{ $vendor->id }}" class="vendor-checkbox">
                                </td>
                                <td class="border px-4 py-2">{{ $vendor->name }}</td>
                                <td class="border px-4 py-2">{{ $vendor->address }}</td>
                                <td class="border px-4 py-2">{{ $vendor->email }}</td>
                                <td class="border px-4 py-2">{{ $vendor->telephone }}</td>
                                <td class="border px-4 py-2">{{ $vendor->pic_name }}</td>
                                <td class="border px-4 py-2 text-center">
                                    <button type="button" onclick="openEditModal({{ $vendor->id }}, '{{ $vendor->email }}', '{{ $vendor->telephone }}', '{{ $vendor->pic_name }}')" class="text-blue-600 hover:underline">✏️ Edit</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
