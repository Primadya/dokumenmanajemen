    ...
    <aside class="w-64 bg-white shadow-md flex flex-col justify-between">
        <div class="p-6">
            ...
            <nav class="space-y-4">
                ...
                <a href="{{ route('admin.eps') }}" class="block text-blue-600 font-semibold">📎 EPS</a>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($eps as $item)
                        <tr class="hover:bg-gray-50 transition duration-200">
                            <td class="px-4 py-3 text-gray-800">{{ $item->eps_number }}</td>
                            <td class="px-4 py-3 text-gray-700">{{ $item->vendor->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-700">{{ \Carbon\Carbon::parse($item->created_at)->format('d/m/Y') }}</td>
